...
/*
 * See regex.h for one further detail of program structure.
 */
              ...
              if (*regparse == '-')
               {
                ...
                if (*regparse == ']' || *regparse == '\0') regc('-');
                else
                 {
                  ...
                  if (class > classend+1) FAIL(RE_INVALIDRANGE);
                  for (; class <= classend; class++) regc((char)class);
                  regparse++;
                 }
               }
              else regc(*regparse++);
 ...
 */
static int  regtry(regexp *prog, const char *string)        /* 0 failure, 1 success */
{
  int i;
  char const **sp;
  char const **ep;
